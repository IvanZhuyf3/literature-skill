...
### Line-Addressable Evidence Chunks

> 来源：[scholaraio](https://github.com/ZimoLiao/scholaraio) `services/chunks.py`

**问题**：`lit search` 命中返回论文级粒度，不知道证据在 full.md 哪个位置。deep research 的来源标注也只能到 `[full: Cheng2015]`，不能到行。

**设计**：parse 完成后自动建 chunk index。

- 按 markdown heading 分节（MinerU 输出已有结构化 heading）
- 段落合并到 ~4800 字符，保留原始 1-indexed `start_line`/`end_line`
- FTS5 `snippet()` 返回高亮片段 + section_title + 行号范围
- `content_hash` 做增量 reindex（只重建改动的 chunk）

```python
@dataclass(frozen=True)
class PaperChunk:
    paper_id: str       # DOI
    section_title: str  # e.g. "Methods"
    seq: int            # chunk 序号
    start_line: int     # ← full.md 的原始行号
    end_line: int
    text: str
    content_hash: str
```

`lit search` 返回格式升级：命中从 `{doi, title, snippet}` → `{doi, title, section, start_line, end_line, snippet}`。

**不需要 vector DB**——纯 FTS5 over chunks，BM25 即可。vector 是另一个 feature，不耦合。
